...
1순위: 포장지에서 직접 읽은 원재료 목록
2순위: 포장지에서 읽을 수 없으면, 해당 제품(브랜드+제품명)의 공식 원재료 정보를 기반으로 작성
3순위: 정확한 제품 정보를 모르면, 해당 사료 유형(건사료/습식 등)과 대상 동물의 일반적인 원재료 구성으로 추정
- 각 항목:
  - name: 원재료명 (한국어)
  - name_en: 원재료명 (영어)
  - order: 표기 순서 (1부터 시작, 함량 많은 순)
  - percentage: 함량 비율 (표기된 경우, 없으면 null)
  - label_name: 포장지에 표기된 그대로의 이름 (읽을 수 없으면 name과 동일하게)
...
추가로 포장지에 표기되어 있거나 제품 정보로 알 수 있는 성분도 모두 포함하세요 (오메가-3, 오메가-6, 타우린, DHA 등).
1순위: 포장지에서 직접 읽은 영양성분 수치
2순위: 해당 제품의 공식 영양성분 정보
3순위: 해당 사료 유형의 일반적인 영양성분 범위로 추정
- 각 항목:
  - name: 성분명 (한국어, 예: "조단백질")
...
# 출력 규칙
- 반드시 JSON 형식으로만 응답
- ⚠️ ingredients와 nutrients는 절대 빈 배열([])이면 안 됩니다. 이미지에서 읽을 수 없더라도 반드시 제품 지식이나 일반적인 사료 정보를 기반으로 채워야 합니다.
...
- ingredients의 order는 포장지 표기 순서 그대로 (추정 시 함량 순)
